...
function ProjectedCostsandSavings() {
    ...
    expenseIds.forEach(expenseId => {
        ...
        // Select the radio button based on the expenseId
        const radioSelector = `input[name="${expenseId.toLowerCase()}_button"]:checked`;
        const frequencyRadioButton = document.querySelector(radioSelector);
    
        if (frequencyRadioButton) {
            const frequency = frequencyRadioButton.value;
    ...
    if(DailyOver >= 0){
        ...
        WithPossibleSavings = projectedCostsWithSavings;
    ...
    const summaryElement = document.querySelector("#summary").innerText = "";
...
function drawChart(timeframe = 'daily') {
    ...
    //const adjustedSavings = totalPossibleSavings / duration * divisor;
    const adjustedSavings = WithPossibleSavings / duration * divisor;
    const data = {
        labels: ['Costs'],
        datasets: [
            {
